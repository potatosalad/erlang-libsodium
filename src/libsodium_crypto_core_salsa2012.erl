...
%%%-------------------------------------------------------------------
-module(libsodium_crypto_core_salsa2012).

-define(NAMESPACE, crypto_core_salsa2012).
...
-export([keybytes/0]).
-export([constbytes/0]).
-export([crypto_core_salsa2012/3]).
...
crypto_core_salsa2012(In, K, C)
		when is_binary(In)
		andalso is_binary(K)
		andalso is_binary(C) ->
	call(crypto_core_salsa2012, {In, K, C}).
