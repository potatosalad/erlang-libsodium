...
%%%-------------------------------------------------------------------
-module(libsodium_crypto_stream_salsa20).

-define(NAMESPACE, crypto_stream_salsa20).

%% API
-export([keybytes/0]).
-export([noncebytes/0]).
-export([messagebytes_max/0]).
-export([crypto_stream_salsa20/3]).
-export(['xor'/3]).
-export([xor_ic/4]).
-export([keygen/0]).
...
crypto_stream_salsa20(CLen, N, K)
		when is_integer(CLen)
		andalso is_binary(N)
		andalso is_binary(K) ->
	call(crypto_stream_salsa20, {CLen, N, K}).
