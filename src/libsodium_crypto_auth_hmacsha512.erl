...
%%%-------------------------------------------------------------------
-module(libsodium_crypto_auth_hmacsha512).

-define(NAMESPACE, crypto_auth_hmacsha512).

%% API
-export([bytes/0]).
-export([keybytes/0]).
-export([crypto_auth_hmacsha512/2]).
-export([verify/3]).
-export([statebytes/0]).
-export([init/1]).
-export([update/2]).
...
crypto_auth_hmacsha512(In, K)
		when is_binary(In)
		andalso is_binary(K) ->
	call(crypto_auth_hmacsha512, {In, K}).
