...
%%%-------------------------------------------------------------------
-module(libsodium_crypto_scalarmult_curve25519).

-define(NAMESPACE, crypto_scalarmult_curve25519).

%% API
-export([bytes/0]).
-export([scalarbytes/0]).
-export([crypto_scalarmult_curve25519/2]).
-export([base/1]).
...
crypto_scalarmult_curve25519(N, P)
		when is_binary(N)
		andalso is_binary(P) ->
	call(crypto_scalarmult_curve25519, {N, P}).
