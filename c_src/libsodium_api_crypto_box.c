...
libsodium_function_t	libsodium_functions_crypto_box[] = {
	LS_API_R_ARG0(crypto_box, seedbytes),
...
LS_API_EXEC(crypto_box, seed_keypair)
{
	...
	LS_SAFE_REPLY(crypto_box_seed_keypair(pk, sk, argv->seed), LS_PROTECT({
		LS_RES_TAG(request),
...
LS_API_EXEC(crypto_box, keypair)
{
	...
	LS_SAFE_REPLY(crypto_box_keypair(pk, sk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(pk), publickeybytes,
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(sk), secretkeybytes,
		ERL_DRV_TUPLE, 2,
...
LS_API_EXEC(crypto_box, easy)
{
	...
	LS_SAFE_REPLY(crypto_box_easy(c, argv->m, argv->mlen, argv->n, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
...
LS_API_EXEC(crypto_box, open_easy)
{
	...
	LS_SAFE_REPLY(crypto_box_open_easy(m, argv->c, argv->clen, argv->n, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
...
LS_API_EXEC(crypto_box, detached)
{
	...
	LS_SAFE_REPLY(crypto_box_detached(c, mac, argv->m, argv->mlen, argv->n, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(c), clen,
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(mac), macbytes,
		ERL_DRV_TUPLE, 2,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_INIT(crypto_box, open_detached)
{
	...
	x = (ErlDrvSizeT)(clen + macbytes + noncebytes + publickeybytes + secretkeybytes + (sizeof (LS_API_F_ARGV_T(crypto_box, open_detached))));
	p = (void *)(driver_alloc(x));
...
LS_API_EXEC(crypto_box, open_detached)
{
	...
	LS_SAFE_REPLY(crypto_box_open_detached(m, argv->c, argv->mac, argv->clen, argv->n, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(m), mlen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, beforenm)
{
	...
	LS_SAFE_REPLY(crypto_box_beforenm(k, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(k), beforenmbytes,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, easy_afternm)
{
	...
	LS_SAFE_REPLY(crypto_box_easy_afternm(c, argv->m, argv->mlen, argv->n, argv->k), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(c), clen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, open_easy_afternm)
{
	...
	LS_SAFE_REPLY(crypto_box_open_easy_afternm(m, argv->c, argv->clen, argv->n, argv->k), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(m), mlen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, detached_afternm)
{
	...
	LS_SAFE_REPLY(crypto_box_detached_afternm(c, mac, argv->m, argv->mlen, argv->n, argv->k), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(c), clen,
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(mac), macbytes,
		ERL_DRV_TUPLE, 2,
...
LS_API_EXEC(crypto_box, open_detached_afternm)
{
	...
	LS_SAFE_REPLY(crypto_box_open_detached_afternm(m, argv->c, argv->mac, argv->clen, argv->n, argv->k), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(m), mlen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, seal)
{
	...
	LS_SAFE_REPLY(crypto_box_seal(c, argv->m, argv->mlen, argv->pk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(c), clen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, seal_open)
{
	...
	LS_SAFE_REPLY(crypto_box_seal_open(m, argv->c, argv->clen, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(m), mlen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, crypto_box)
{
	...
	LS_SAFE_REPLY(crypto_box(c, argv->m, argv->mlen, argv->n, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(c), clen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, open)
{
	...
	LS_SAFE_REPLY(crypto_box_open(m, argv->c, argv->clen, argv->n, argv->pk, argv->sk), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(m), mlen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, afternm)
{
	...
	LS_SAFE_REPLY(crypto_box_afternm(c, argv->m, argv->mlen, argv->n, argv->k), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(c), clen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
...
LS_API_EXEC(crypto_box, open_afternm)
{
	...
	LS_SAFE_REPLY(crypto_box_open_afternm(m, argv->c, argv->clen, argv->n, argv->k), LS_PROTECT({
		LS_RES_TAG(request),
		ERL_DRV_BUF2BINARY, (ErlDrvTermData)(m), mlen,
		ERL_DRV_TUPLE, 2
	}), __FILE__, __LINE__);
