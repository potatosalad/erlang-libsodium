...
libsodium_function_t	libsodium_functions_crypto_pwhash[] = {
	LS_API_R_ARG0(crypto_pwhash, alg_argon2i13),
...
LS_API_EXEC(crypto_pwhash, crypto_pwhash)
{
	...
	LS_SAFE_REPLY(crypto_pwhash(out, argv->outlen, argv->passwd, argv->passwdlen, argv->salt, argv->opslimit, argv->memlimit, argv->alg), LS_PROTECT({
		LS_RES_TAG(request),
